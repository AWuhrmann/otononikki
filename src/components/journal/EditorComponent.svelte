<script lang="ts">
  ...
  return new Plugin({
    ...
    props: {
      priority: 10000, // Even higher priority
      handleDOMEvents: {
        drop: (view, event) => {
    ...
    const fileDropPlugin = _prose(() => {
      console.log("Creating high-priority file drop plugin");
      ...
      return new Plugin({
        ...
        props: {
          // High priority to handle before other plugins
          priority: 1000,
          
          handleDOMEvents: {
            // Handle dragenter for visual feedback
            dragenter: (view, event) => {
              console.log("File plugin dragenter, types:", event.dataTransfer?.types);
              
              // Check for our custom file data OR native file drops
              const hasCustomData = event.dataTransfer?.types.includes("application/json");
            ...
            dragover: (view, event) => {
              ...
              if (Date.now() - lastDragLog > 100) {
                console.log("File plugin dragover, types:", event.dataTransfer?.types);
            ...
            drop: (view, event) => {
              console.log("File plugin DROP event, types:", event.dataTransfer?.types);
              ...
              try {
                ...
                if (hasCustomData) {
                  const fileDataStr = event.dataTransfer!.getData("application/json");
                  if (fileDataStr) {
                ...
                if (pos && pos.pos >= 0) {
                  ...
                  tr.insertText(markdownToInsert, pos.pos);
                  console.log("Inserting at drop position:", pos.pos);
                } else {
                  // Fallback: insert at current cursor position
                  const { from } = state.selection;
                  tr.insertText(markdownToInsert, from);
                  console.log("Inserting at cursor position:", from);
                }
