<script lang="ts">
  ...
  $effect(() => {
    ...
    if (contextExpanded !== expanded) {
      debugLog(
        `🔄 Syncing expanded state: local ${expanded} → context ${contextExpanded}`,
      );
  ...
  function debugLog(message: string, data?: any) {
    ...
    console.log(
      `${prefix} ${message} [expanded: ${expandedState}, children: ${childrenCount}]`,
      data || "",
    );
  }
  ...
  function handleDragStart(event: DragEvent) {
    ...
    // REMOVE or comment out the text/plain line - this is likely the culprit!
    event.dataTransfer.setData('text/plain', item.id);

    // Keep only the application/json
    ...
    // Set drag effect
    event.dataTransfer.effectAllowed = "copyMove"; // Also change from 'move' to 'copy'
  ...
  async function handleDrop(event: DragEvent) {
    ...
    const draggedItemId = event.dataTransfer.getData("text/plain");
    const draggedItemData = JSON.parse(
...
<div class="tree-node" style="padding-left: {indent}px" bind:this={nodeElement}>
  <div
    class="node-header"
  ...
  {#if expanded}
    {#each children as child (child.id)}
      <TreeNode item={child} level={level + 1} {onFileSelect} />
    {/each}
  {/if}
</div>
