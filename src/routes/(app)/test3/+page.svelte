<script lang="ts">
  import { Label, Switch, TimeRangeField } from "bits-ui";
...
<div class="bg-gray-50 rounded-[10px] w-[200px] m-10 border-1 border-gray-300 p-5">
...
<div>
        ...
        {#snippet children({ segments })}
          {#each segments as { part, value }, i (part + i)}
            <div class="inline-block select-none">
              {#if part === "literal"}
                <DatePicker.Segment {part} class="text-muted-foreground p-1/3">
                  {value}
                </DatePicker.Segment>
              {:else}
                <DatePicker.Segment
  ...
  {#if !myChecked}
  <TimeRangeField.Root hourCycle={24 as const} validate={(timeRange) => {if (timeRange.start && timeRange.end) {
