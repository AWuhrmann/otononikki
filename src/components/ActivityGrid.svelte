<script lang="ts">
    ...
    let { values = [], monthYear, lastValue, color = "#000000", n = 15 } = $props()
    ...
    function processBarChartData(data, n = 15) {
  ...
  return result;
    ...
    function processGridData(data, monthYear) {
      ...
      data.forEach((item) => {
        ...
        if (
          itemMonth === month && 
          itemYear === year && 
          (!valuesByDay.has(day) || new Date(valuesByDay.get(day).timestamp) < date)
        ) {
    ...
    $effect(() => {
      ...
      if (!isHovered) {
        ...
        const xScale = d3
          .scaleBand()
          .domain(barData.map((d, i) => i.toString()))
          .range([0, width])
          .padding(0.2)
